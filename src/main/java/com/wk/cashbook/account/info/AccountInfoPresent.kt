...
import com.wk.cashbook.account.UpdateAccountOrWalletActivity
import com.wk.cashbook.account.UpdateAccountOrWalletActivity.Companion.DATA_TYPE
import com.wk.cashbook.account.UpdateAccountOrWalletActivity.Companion.TYPE_WALLET
import com.wk.cashbook.account.wallet.WalletTradeRecodeActivity
...
class AccountInfoPresent(private val mAccountInfoActivity: AccountInfoActivity) : BaseProjectsPresent() {
    ...
    fun gotoWalletRecodeInfo(walletId:Long){
        val intent = Intent(mAccountInfoActivity, WalletTradeRecodeActivity::class.java)
        intent.putExtra(AccountWallet.ACCOUNT_WALLET_ID,walletId)
        mAccountInfoActivity.startActivityForResult(intent, CashBookActivityRequestCode.REQUEST_CODE_ACCOUNT_LIST_ACTIVITY)
    }
}