...
public class CashBookSql {
    ...
    public static final String SQL_QUERY_WALLET_TRADE_RECODE="select id,tradetime,amount,tradenote,(case when accountid =? and categoryId!=? then \"yes\" else \"no\" end) as pay ,(select amount from accountwallet where id=?) as balance from traderecode\n" +
